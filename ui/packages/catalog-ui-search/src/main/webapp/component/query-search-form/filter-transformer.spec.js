...
describe('flatten', () => {
  ...
  it('converts AND to multiple filters', () => {
      expect(flatten(andFilter)).to.deep.equal([basicFilter, multivaluedChecksumFilter])
  }) 
})
...
describe('expand', () => {
    ...
    it('converts multiple filters to AND', () => {
        expect(expand([basicFilter, multivaluedChecksumFilter])).to.deep.equal(andFilter)
    })
